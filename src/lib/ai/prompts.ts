...
6. 右侧文案要短但完整，必须解释“为什么”，不要只写结论；每个框控制在 1-3 行
7. 正确思路必须恰好 3 步，按解题顺序写具体动作
8. 画面使用中文大字，文字要清晰、不要错字、不要水印、不要多余品牌、不要出现题目完整长文本和全部选项
9. 严格返回 JSON，不要 markdown，不要代码块
...
学生作答：${userAnswers[String(subQuestion.subNumber)] ?? ''}`
  )
  .join('\n\n')}`;
}
